...
class Page:
    ...
    def title(self) -> str:
        ...
        return self.frontmatter.get("title", self.source_path.stem.replace("-", " ").title())
...
class Category:
    ...
    def blog_posts(self) -> list[Page]:
        ...
        posts.sort(
            key=lambda p: p.frontmatter.get("date", "0000-00-00"),
            reverse=True
        )
        return posts
...
class Site:
    ...
    categories: dict[str, Category] = field(default_factory=dict)  # Category name → Category
...
def validate_site_structure(site: Site) -> list[str]:
    ...
    if site.orphaned_pages:
        warnings.append(
            f"Found {len(site.orphaned_pages)} orphaned pages "
            "(not in any category)"
        )
    ...
    for cat_name, category in site.categories.items():
        if not category.pages:
            warnings.append(
                f"Category '{cat_name}' has no pages (only root page)"
            )
