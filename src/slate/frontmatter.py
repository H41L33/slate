...
try:
    ...
    raise ImportError(
        "PyYAML is required for frontmatter support. Install with: pip install pyyaml"
    ) from exc
...
def validate_frontmatter(metadata: dict[str, Any], file_path: str) -> list[str]:
    ...
    # If type is "blog-post", require date and title
    if metadata.get("type") == "blog-post":
...
def merge_with_cli_args(
    frontmatter: dict[str, Any], cli_args: dict[str, Any]
) -> dict[str, Any]:
