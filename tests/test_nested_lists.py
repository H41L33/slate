...
"""
    ...
    assert unordered_list_blocks, (
        "Expected at least one unordered list block in the parsed output."
    )
    ...
    assert isinstance(first_item, dict), (
        "First item should be a dictionary for structured content."
    )
    assert first_item.get("p") == "Parent A", (
        "Expected 'Parent A' as the paragraph content."
    )
    assert "ul" in first_item, "Expected a nested unordered list within 'Parent A'."
    assert first_item["ul"] == ["Child A1", "Child A2"], (
        "Expected 'Child A1' and 'Child A2' as nested list items."
    )

    # --- Verify the second top-level list item (Parent B) ---
    second_item = list_items[1]
    assert isinstance(second_item, dict), (
        "Second item should be a dictionary for structured content."
    )
    assert second_item.get("p") == "Parent B", (
        "Expected 'Parent B' as the paragraph content."
    )
    ...
    assert isinstance(first_ordered_list_item, dict), (
        "First item of nested ordered list should be a dictionary."
    )
    assert first_ordered_list_item.get("p") == "Sub B1", (
        "Expected 'Sub B1' as the paragraph content."
    )
    assert "ul" in first_ordered_list_item, (
        "Expected a nested unordered list within 'Sub B1'."
    )
    assert first_ordered_list_item["ul"] == ["Subsub B1a"], (
        "Expected 'Subsub B1a' as a nested list item."
    )
...
"""
    ...
    assert "<li class='content-li'>" in generated_html
